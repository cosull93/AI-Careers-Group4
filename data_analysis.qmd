...
# Import Data
Import the dataset using Pyspark. Start a spark session, load to a data frame, and register.
...
### Review the numeric variables in a matrix to understand how the NAs are coorelated to each other. A value of 1 means that whenever one variable is missing the other is also missing. A value of -1 infers the opposite, and a value of 0 infers independence in the missingness of the variables. 

### This matrix reveals two correlation groups. One relating the company and qalitative information about the job and another group related to salary to and from. The rest of the variables have NA records independant of one another. The cooralation of missing to and from dates are more than likely associated the the fact that some states require this information on job postings while others do not. The perfect positive coorelation for company, career area, lot occupation, lot specialized occupation, lot occupation group,  NAICS 2022_2, 2022_3, 2022_4, 2022_5, and 2022_6 suggests they are all part of the same survey or survey section and these answers may require all of these fields to be entered in order to submit or the data has already been cleaned for this group.
```{python}
...
#pandas_num.head()
```
### For now, consider NA coorelations as context. replace missing values with the respective medians.


## Handling Missing Values(Categorical Python Implementation)

### Review the categoric variables in a matrix to understand how the NAs are coorelated to each other. A value of 1 means that whenever one variable is missing the other is also missing. A value of -1 infers the opposite, and a value of 0 infers independence in the missingness of the variables. 

### Several variables have positive NA coorelations of 1. The exceptions are max edu levels, original pay period, title, and msa name. The rest are always NA when one of the other values in the same group are. Replace all NAs of categorical variables. 
...
categorical_cols = [
    ...
    "NAICS_2022_5_NAME", "NAICS_2022_6_NAME","SPECIALIZED_SKILLS_NAME","CERTIFICATIONS_NAME","COMMON_SKILLS_NAME","SOFTWARE_SKILLS_NAME","LOT_CAREER_AREA", "LOT_OCCUPATION", "LOT_SPECIALIZED_OCCUPATION",
...
### Posted and expired date NAs are independant of each other. 
...
### Combine the data frames for the thre variable types.

```{python}
import pandas as pd

# Reset indices before concatenating 
pandas_num = pandas_num.reset_index(drop=True)
pandas_cat = pandas_cat.reset_index(drop=True)
pandas_date = pandas_date.reset_index(drop=True)
...
print(f"Total missing: {pandas_handled_missing_values.isnull().sum().sum()}")
...
completion_rate = (1 - pandas_cleaned.isnull().sum() / len(pandas_cleaned)) * 100

plt.figure(figsize=(12, 8))
completion_rate.sort_values().plot(kind='barh', color='green')
plt.xlabel('Completion Rate (%)')
plt.title('Data Completeness After Cleaning')
plt.axvline(x=100, color='darkgreen', linestyle='--', linewidth=2, label='100% Complete')
plt.xlim(95, 101)
plt.legend()
plt.tight_layout()
plt.show()
...
```
## An exploratoy analysis of ai and non ai trends
...
### Science and technical services more than double the next job posting group by industry. Six industries comprise 80% of job postings. Many non-AI roles fall into the Professional, Scientific, and Technical Services category. The example used in the overview section was engineers. The example of highly automatable jobs included retail and financial services, which does lag the top category significantly.
```{python}
# running percent of total
industry_counts['cumPercent'] = industry_counts['Count'].cumsum()/industry_counts['Count'].sum()*100

# Step 4: Plot
fig, ax1 = plt.subplots(figsize=(8,6))

# Bar plot (frequencies)
sns.barplot(x="Industry", y="Count", data=industry_counts, ax=ax1, palette="colorblind")
ax1.set_ylabel("number of jobs by industry")
ax1.tick_params(axis="x", rotation=90)

# Line plot (cumulative %)
ax2 = ax1.twinx()
sns.lineplot(x="Industry", y="cumPercent", data=industry_counts, ax=ax2, color="black", marker="o")
ax2.set_ylabel("Cumulative %")

# Add horizontal reference line at 80% (Pareto rule)
ax2.axhline(80, color="black", linestyle="--")
plt.title("Pareto Chart of Industries Job Postings")
plt.tight_layout()
plt.show()
```

### Next review the data to understand the trends in AI mentioned as a software skill. To do this search for the mention of "A.I." in the software skills. This is a relative measure as other text could describe software skills relative to A.I.. The result shows a significant mention of AI mentions in job skill requirements. 
 ```{python}
pandas_cleaned["aiMentioned"] = pandas_cleaned["SOFTWARE_SKILLS_NAME"].str.contains("a.i.", case=False, na=False)
# Plot
...
plt.title("Trends in job hiring speed given A.I. skills requirements")
plt.xlabel("DURATION")
plt.ylabel("Salary")  
plt.tight_layout()
plt.show()
...
#plt.pie(
#    industry_counts.values,           # sizes
#    labels=industry_counts.index,
#    labeldistance=1.2,     # labels
...
```{python}
pandas_cleaned.columns
```


```{python}
sns.lineplot(data=pandas_cleaned, x="NAICS_2022_2_NAME", y="SALARY", hue="aiMentioned", alpha=0.1)

plt.legend(title="Which Industries pay more and require the most experience", loc="upper left", fontsize=3, title_fontsize=6, ncol=3)
