...
```
## EDA Focus on job postings, salary trends, and geographic variations.
```{python}
...
# Science and technical services more than double the next job posting group by industry. Six industries comprise 80% of job postings.
```{python}
...
sns.barplot(x="Industry", y="Count", data=industry_counts, ax=ax1, color="black")
ax1.set_ylabel("number of jobs by industry")
ax1.tick_params(axis="x", rotation=90)

# Line plot (cumulative %)
ax2 = ax1.twinx()
sns.lineplot(x="Industry", y="cumPercent", data=industry_counts, ax=ax2, color="purple", marker="o")
ax2.set_ylabel("Cumulative %")

# Add horizontal reference line at 80% (Pareto rule)
ax2.axhline(80, color="gold", linestyle="--")
